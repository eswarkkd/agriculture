	...
	constructor(props){
		...
		<div className="container-fluid main_div_new">
			...
			<div className="row no-margin">
				...
				<div className="col-sm-10 text-center">
					<div className="row">
						<div className="col-sm-12 m-2"><h3>Edit Purchase</h3><br /></div>
					</div>
					<div className="row">
						...
						<select name="former_name" className="form-control" value={this.state.former_id} onChange={(e)=>this.setState({former_id:e.target.value})}>
						{(this.state.formers_list)?this.state.formers_list.map((item,i)=>(<option value={item.id} key={i}>{item.first_name} {item.last_name}</option>)):""}							
											...
											<div className="col-sm-2">
												<select name="item_name" className="form-control" value={this.state.item_name} onChange={(e)=>this.setState({item_name:e.target.value})}>
											...
											<div className="col-sm-2">
												<select name="quantity_type" className="form-control" value={this.state.quantity_type} onChange={(e)=>this.setState({quantity_type:e.target.value})}>
								...
								<div className="row">
									<div className="col-sm-12"><br />
										<div className="row">
											...
											<div className="col-sm-2">
												<input type="text" name="quantity" className="form-control" value={this.state.quantity} onChange={(e)=>this.setState({quantity:e.target.value})} />
											</div>
											<div className="col-sm-2 label_class">Item Price</div>
											<div className="col-sm-2"><input type="text" name="item_price" className="form-control" value={this.state.item_price} onChange={(e)=>this.setState({item_price:e.target.value})} /></div>
											<div className="col-sm-2 label_class">Amount</div>
											<div className="col-sm-2">
											<input type="text" name="amount" className="form-control" value={this.state.amount} onChange={(e)=>this.setState({amount:e.target.value})} />
											<input type="hidden" name="purchase_id" className="form-control" value={this.state.purchase_id} />
											</div>
										</div>
									</div>
								</div>
	 ...
	 componentDidMount(){
		// formers list
		 ...
		 // formers list
		 fetch("http://127.0.0.1:3001/get_purchase_by_id",{
		 method:"POST",
			 headers:{
				 "Accept":"text/plain",
				 "Content-Type":"application/x-www-form-urlencoded"
			 },
			 body:"results_from=0&results_to=10&purchase_id="+window.edit_purchase+"&"
		 ...
		 .then((responsive_data)=>{
			 if(responsive_data.status==1){
				 this.setState({former_id:responsive_data.result[0].former_id,item_name:responsive_data.result[0].item_name,quantity_type:responsive_data.result[0].quantity_type,quantity:responsive_data.result[0].quantity,item_price:responsive_data.result[0].item_price,amount:responsive_data.result[0].amount,purchase_id:responsive_data.result[0].id});
			 }else{
	 ...
	 submit_form(e){
		 ...
		 fetch("http://127.0.0.1:3001/edit_purchase",{
		 method:"POST",
			 headers:{
				 "Accept":"text/plain",
				 "Content-Type":"application/x-www-form-urlencoded"
			 },
			 body:"former_id="+data.former_name+"&item_name="+data.item_name+"&quantity="+data.quantity+"&quantity_type="+data.quantity_type+"&item_price="+data.item_price+"&amount="+data.amount+"&purchase_id="+data.purchase_id+"&"
